...
2. `indexer_rpc_rate_limit_waits_total` 메트릭으로 레이트 리밋 확인
3. endpoint failover 또는 요청 간격 완화
4. 체인별 RPC rate limit 조정 (`SOLANA_RPC_RATE_LIMIT`, `BASE_RPC_RATE_LIMIT` 등)
5. fetch workers와 batch size 임시 조정

### B. Sidecar Decode Failures
1. 실패 트랜잭션 샘플 추출
2. 체인별로 분리 진단 (`solana`, `base`, `btc`) 후 blast radius 확인
3. OTel 트레이스로 `normalizer.grpcDecode` 스팬 확인
4. 디코더 로직 회귀 여부 확인
5. hotfix PR 생성 후 CI 통과 확인, 단계적 배포
