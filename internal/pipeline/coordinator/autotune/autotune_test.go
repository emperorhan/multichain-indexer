...
func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	reintegrationSealDriftReanchorCompaction2Digest := reintegrationSealDriftReanchor2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-epoch=14"
	reintegrationSealDriftReanchorCompactionExpiry2Digest := reintegrationSealDriftReanchorCompaction2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-epoch=16"
	reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest := reintegrationSealDriftReanchorCompactionExpiry2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=18"

	reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration1Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration1Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=19"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=20"
	staleReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg := segment2Cfg
	staleReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=19"
	stalePreReintegrationWithAdvancedQuarantineCfg := segment2Cfg
	stalePreReintegrationWithAdvancedQuarantineCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiry2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=21"
	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg := segment2Cfg
	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiry2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=21"
	conflictingReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationAliasForwardCfg := segment2Cfg
	conflictingReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationAliasForwardCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest +
		"|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=20|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=19"
	conflictingReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationAliasReverseCfg := segment2Cfg
	conflictingReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationAliasReverseCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest +
		"|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=19|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=20"
	stalePreReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg := segment2Cfg
	stalePreReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest

	baseSeed := 140
	baseController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg, &baseSeed)
	require.NotNil(t, baseController)
	batch, baseDecision := baseController.Resolve(highLag)
	assert.Equal(t, baseSeed+20, batch)
	assert.Equal(t, "apply_increase", baseDecision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestDigest, baseDecision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestRefreshEpoch, baseDecision.PolicyEpoch)
	assert.Equal(t, 0, baseDecision.PolicyActivationTicks)

	reintegration1Seed := baseController.currentBatch
	reintegration1Controller := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration1Cfg, &reintegration1Seed)
	require.NotNil(t, reintegration1Controller)
	reintegration1Controller.reconcilePolicyTransition(baseController.exportPolicyTransition())
	batch, reintegration1Decision := reintegration1Controller.Resolve(highLag)
	assert.Equal(t, reintegration1Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegration1Decision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration1Cfg.PolicyManifestDigest, reintegration1Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration1Cfg.PolicyManifestRefreshEpoch, reintegration1Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegration1Decision.PolicyActivationTicks)

	reintegration2Seed := reintegration1Controller.currentBatch
	reintegration2Controller := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg, &reintegration2Seed)
	require.NotNil(t, reintegration2Controller)
	reintegration2Controller.reconcilePolicyTransition(reintegration1Controller.exportPolicyTransition())
	batch, reintegration2Decision := reintegration2Controller.Resolve(highLag)
	assert.Equal(t, reintegration2Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegration2Decision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest, reintegration2Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, reintegration2Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegration2Decision.PolicyActivationTicks)

	staleReintegrationSeed := reintegration2Controller.currentBatch
	staleReintegrationController := newAutoTuneControllerWithSeed(100, staleReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg, &staleReintegrationSeed)
	require.NotNil(t, staleReintegrationController)
	staleReintegrationController.reconcilePolicyTransition(reintegration2Controller.exportPolicyTransition())
	batch, staleReintegrationDecision := staleReintegrationController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSeed+20, batch)
	assert.Equal(t, "apply_increase", staleReintegrationDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
		staleReintegrationDecision.PolicyManifestDigest,
		"lower reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration epochs must remain pinned behind latest verified reintegration ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, staleReintegrationDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationDecision.PolicyActivationTicks)

	stalePreReintegrationWithAdvancedQuarantineSeed := staleReintegrationController.currentBatch
	stalePreReintegrationWithAdvancedQuarantineController := newAutoTuneControllerWithSeed(100, stalePreReintegrationWithAdvancedQuarantineCfg, &stalePreReintegrationWithAdvancedQuarantineSeed)
	require.NotNil(t, stalePreReintegrationWithAdvancedQuarantineController)
	stalePreReintegrationWithAdvancedQuarantineController.reconcilePolicyTransition(staleReintegrationController.exportPolicyTransition())
	batch, stalePreReintegrationWithAdvancedQuarantineDecision := stalePreReintegrationWithAdvancedQuarantineController.Resolve(highLag)
	assert.Equal(t, stalePreReintegrationWithAdvancedQuarantineSeed+20, batch)
	assert.Equal(t, "apply_increase", stalePreReintegrationWithAdvancedQuarantineDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
		stalePreReintegrationWithAdvancedQuarantineDecision.PolicyManifestDigest,
		"post-reintegration ownership must not regress to pre-reintegration quarantine even when stale quarantine epochs are numerically higher",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, stalePreReintegrationWithAdvancedQuarantineDecision.PolicyEpoch)
	assert.Equal(t, 0, stalePreReintegrationWithAdvancedQuarantineDecision.PolicyActivationTicks)

	ambiguousReintegrationSeed := staleReintegrationController.currentBatch
	ambiguousReintegrationController := newAutoTuneControllerWithSeed(100, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg, &ambiguousReintegrationSeed)
	require.NotNil(t, ambiguousReintegrationController)
	ambiguousReintegrationController.reconcilePolicyTransition(staleReintegrationController.exportPolicyTransition())
	batch, ambiguousReintegrationDecision := ambiguousReintegrationController.Resolve(highLag)
	assert.Equal(t, ambiguousReintegrationSeed+20, batch)
	assert.Equal(t, "apply_increase", ambiguousReintegrationDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
		ambiguousReintegrationDecision.PolicyManifestDigest,
		"reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration candidates must remain quarantined until late-resurrection-quarantine ownership is explicit",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationDecision.PolicyActivationTicks)

	conflictingForwardSeed := ambiguousReintegrationController.currentBatch
	conflictingForwardController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationAliasForwardCfg, &conflictingForwardSeed)
	require.NotNil(t, conflictingForwardController)
	conflictingForwardController.reconcilePolicyTransition(ambiguousReintegrationController.exportPolicyTransition())
	batch, conflictingForwardDecision := conflictingForwardController.Resolve(highLag)
	assert.Equal(t, conflictingForwardSeed+20, batch)
	assert.Equal(t, "apply_increase", conflictingForwardDecision.Decision)
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
		conflictingForwardDecision.PolicyManifestDigest,
		"conflicting reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration alias values must remain quarantined regardless of token order",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, conflictingForwardDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingForwardDecision.PolicyActivationTicks)

	conflictingReverseSeed := conflictingForwardController.currentBatch
	conflictingReverseController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationAliasReverseCfg, &conflictingReverseSeed)
	require.NotNil(t, conflictingReverseController)
	conflictingReverseController.reconcilePolicyTransition(conflictingForwardController.exportPolicyTransition())
	batch, conflictingReverseDecision := conflictingReverseController.Resolve(highLag)
	assert.Equal(t, conflictingReverseSeed+20, batch)
	assert.Equal(t, "apply_increase", conflictingReverseDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
	...
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, conflictingReverseDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingReverseDecision.PolicyActivationTicks)

	stalePreReintegrationSeed := conflictingReverseController.currentBatch
	stalePreReintegrationController := newAutoTuneControllerWithSeed(100, stalePreReintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationCfg, &stalePreReintegrationSeed)
	require.NotNil(t, stalePreReintegrationController)
	stalePreReintegrationController.reconcilePolicyTransition(conflictingReverseController.exportPolicyTransition())
	batch, stalePreReintegrationDecision := stalePreReintegrationController.Resolve(highLag)
	assert.Equal(t, stalePreReintegrationSeed+20, batch)
	assert.Equal(t, "apply_increase", stalePreReintegrationDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
		stalePreReintegrationDecision.PolicyManifestDigest,
		"post-reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine stale markers must not reclaim ownership after reintegration",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, stalePreReintegrationDecision.PolicyEpoch)
	assert.Equal(t, 0, stalePreReintegrationDecision.PolicyActivationTicks)

	crashRestartSeed := stalePreReintegrationController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
		Version:                 reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyVersion,
		ManifestDigest:          reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest,
		Epoch:                   reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch,
		ActivationHoldRemaining: 2,
		FromWarmCheckpoint:      true,
	})
	batch, crashRestartHold := crashRestartController.Resolve(highLag)
	...
	assert.Equal(
		...
		1,
		crashRestartHold.PolicyActivationTicks,
		"crash-restart at reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration boundary must collapse ambiguous hold windows to one deterministic hold tick",
	)
	batch, crashRestartApplied := crashRestartController.Resolve(highLag)
	assert.Equal(t, crashRestartSeed+20, batch)
	assert.Equal(t, "apply_increase", crashRestartApplied.Decision)
	...
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
	assert.Equal(t, segment3Cfg.PolicyManifestRefreshEpoch, reForwardHold.PolicyEpoch)
	assert.Equal(t, 1, reForwardHold.PolicyActivationTicks)
}

func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	reintegrationSealDriftReanchorCompactionExpiry2Digest := reintegrationSealDriftReanchorCompaction2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-epoch=16"
	reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest := reintegrationSealDriftReanchorCompactionExpiry2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=18"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=20"

	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest
	reintegrationSealAfterReintegration1Cfg := segment2Cfg
	reintegrationSealAfterReintegration1Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=21"
	reintegrationSealAfterReintegration2Cfg := segment2Cfg
	reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=22"
	staleReintegrationSealAfterReintegrationCfg := segment2Cfg
	staleReintegrationSealAfterReintegrationCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=21"
	ambiguousReintegrationSealAfterReintegrationCfg := segment2Cfg
	ambiguousReintegrationSealAfterReintegrationCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=23"
	conflictingReintegrationSealAfterReintegrationAliasForwardCfg := segment2Cfg
	conflictingReintegrationSealAfterReintegrationAliasForwardCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest +
		"|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=22|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=21"
	conflictingReintegrationSealAfterReintegrationAliasReverseCfg := segment2Cfg
	conflictingReintegrationSealAfterReintegrationAliasReverseCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest +
		"|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=21|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=22"
	stalePreReintegrationSealAfterReintegrationCfg := segment2Cfg
	stalePreReintegrationSealAfterReintegrationCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest

	baseSeed := 140
	baseController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg, &baseSeed)
	require.NotNil(t, baseController)
	batch, baseDecision := baseController.Resolve(highLag)
	assert.Equal(t, baseSeed+20, batch)
	assert.Equal(t, "apply_increase", baseDecision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest, baseDecision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestRefreshEpoch, baseDecision.PolicyEpoch)
	assert.Equal(t, 0, baseDecision.PolicyActivationTicks)

	reintegrationSeal1Seed := baseController.currentBatch
	reintegrationSeal1Controller := newAutoTuneControllerWithSeed(100, reintegrationSealAfterReintegration1Cfg, &reintegrationSeal1Seed)
	require.NotNil(t, reintegrationSeal1Controller)
	reintegrationSeal1Controller.reconcilePolicyTransition(baseController.exportPolicyTransition())
	batch, reintegrationSeal1Decision := reintegrationSeal1Controller.Resolve(highLag)
	assert.Equal(t, reintegrationSeal1Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegrationSeal1Decision.Decision)
	assert.Equal(t, reintegrationSealAfterReintegration1Cfg.PolicyManifestDigest, reintegrationSeal1Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealAfterReintegration1Cfg.PolicyManifestRefreshEpoch, reintegrationSeal1Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegrationSeal1Decision.PolicyActivationTicks)

	reintegrationSeal2Seed := reintegrationSeal1Controller.currentBatch
	reintegrationSeal2Controller := newAutoTuneControllerWithSeed(100, reintegrationSealAfterReintegration2Cfg, &reintegrationSeal2Seed)
	require.NotNil(t, reintegrationSeal2Controller)
	reintegrationSeal2Controller.reconcilePolicyTransition(reintegrationSeal1Controller.exportPolicyTransition())
	batch, reintegrationSeal2Decision := reintegrationSeal2Controller.Resolve(highLag)
	assert.Equal(t, reintegrationSeal2Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegrationSeal2Decision.Decision)
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest, reintegrationSeal2Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch, reintegrationSeal2Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegrationSeal2Decision.PolicyActivationTicks)

	staleReintegrationSealSeed := reintegrationSeal2Controller.currentBatch
	staleReintegrationSealController := newAutoTuneControllerWithSeed(100, staleReintegrationSealAfterReintegrationCfg, &staleReintegrationSealSeed)
	require.NotNil(t, staleReintegrationSealController)
	staleReintegrationSealController.reconcilePolicyTransition(reintegrationSeal2Controller.exportPolicyTransition())
	batch, staleReintegrationSealDecision := staleReintegrationSealController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSealSeed+20, batch)
	...
	assert.Equal(
		t,
		reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest,
		staleReintegrationSealDecision.PolicyManifestDigest,
		"lower reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration-seal epochs must remain pinned behind latest verified reintegration-seal ownership",
	)
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDecision.PolicyActivationTicks)

	ambiguousReintegrationSealSeed := staleReintegrationSealController.currentBatch
	ambiguousReintegrationSealController := newAutoTuneControllerWithSeed(100, ambiguousReintegrationSealAfterReintegrationCfg, &ambiguousReintegrationSealSeed)
	require.NotNil(t, ambiguousReintegrationSealController)
	ambiguousReintegrationSealController.reconcilePolicyTransition(staleReintegrationSealController.exportPolicyTransition())
	batch, ambiguousReintegrationSealDecision := ambiguousReintegrationSealController.Resolve(highLag)
	assert.Equal(t, ambiguousReintegrationSealSeed+20, batch)
	...
	assert.Equal(
		...
		reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest,
		ambiguousReintegrationSealDecision.PolicyManifestDigest,
		"reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration-seal candidates must remain quarantined until reintegration ownership is explicit",
	)
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationSealDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationSealDecision.PolicyActivationTicks)

	conflictingForwardSeed := ambiguousReintegrationSealController.currentBatch
	conflictingForwardController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealAfterReintegrationAliasForwardCfg, &conflictingForwardSeed)
	require.NotNil(t, conflictingForwardController)
	conflictingForwardController.reconcilePolicyTransition(ambiguousReintegrationSealController.exportPolicyTransition())
	batch, conflictingForwardDecision := conflictingForwardController.Resolve(highLag)
	assert.Equal(t, conflictingForwardSeed+20, batch)
	assert.Equal(t, "apply_increase", conflictingForwardDecision.Decision)
	assert.Equal(
		...
		reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest,
		conflictingForwardDecision.PolicyManifestDigest,
		"conflicting reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration-seal alias values must remain quarantined regardless of token order",
	)
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch, conflictingForwardDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingForwardDecision.PolicyActivationTicks)

	conflictingReverseSeed := conflictingForwardController.currentBatch
	conflictingReverseController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealAfterReintegrationAliasReverseCfg, &conflictingReverseSeed)
	require.NotNil(t, conflictingReverseController)
	conflictingReverseController.reconcilePolicyTransition(conflictingForwardController.exportPolicyTransition())
	batch, conflictingReverseDecision := conflictingReverseController.Resolve(highLag)
	assert.Equal(t, conflictingReverseSeed+20, batch)
	...
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch, conflictingReverseDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingReverseDecision.PolicyActivationTicks)

	stalePreReintegrationSealSeed := conflictingReverseController.currentBatch
	stalePreReintegrationSealController := newAutoTuneControllerWithSeed(100, stalePreReintegrationSealAfterReintegrationCfg, &stalePreReintegrationSealSeed)
	require.NotNil(t, stalePreReintegrationSealController)
	stalePreReintegrationSealController.reconcilePolicyTransition(conflictingReverseController.exportPolicyTransition())
	batch, stalePreReintegrationSealDecision := stalePreReintegrationSealController.Resolve(highLag)
	assert.Equal(t, stalePreReintegrationSealSeed+20, batch)
	assert.Equal(t, "apply_increase", stalePreReintegrationSealDecision.Decision)
	assert.Equal(
		...
		reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest,
		stalePreReintegrationSealDecision.PolicyManifestDigest,
		"post-reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration stale markers must not reclaim ownership after reintegration-seal",
	)
	assert.Equal(t, reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch, stalePreReintegrationSealDecision.PolicyEpoch)
	assert.Equal(t, 0, stalePreReintegrationSealDecision.PolicyActivationTicks)

	crashRestartSeed := stalePreReintegrationSealController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealAfterReintegration2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
		Version:                 reintegrationSealAfterReintegration2Cfg.PolicyVersion,
		ManifestDigest:          reintegrationSealAfterReintegration2Cfg.PolicyManifestDigest,
		Epoch:                   reintegrationSealAfterReintegration2Cfg.PolicyManifestRefreshEpoch,
		ActivationHoldRemaining: 2,
		FromWarmCheckpoint:      true,
	})
	batch, crashRestartHold := crashRestartController.Resolve(highLag)
	...
	assert.Equal(
		...
		1,
		crashRestartHold.PolicyActivationTicks,
		"crash-restart at reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration-seal boundary must collapse ambiguous hold windows to one deterministic hold tick",
	)
	batch, crashRestartApplied := crashRestartController.Resolve(highLag)
	assert.Equal(t, crashRestartSeed+20, batch)
	assert.Equal(t, "apply_increase", crashRestartApplied.Decision)
	...
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration-seal must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
	assert.Equal(t, segment3Cfg.PolicyManifestRefreshEpoch, reForwardHold.PolicyEpoch)
	assert.Equal(t, 1, reForwardHold.PolicyActivationTicks)
}

func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	lateResurrection2Digest := "manifest-tail-v2b" +
		...
		"|rollback-fence-post-marker-expiry-late-resurrection-quarantine-epoch=4"
	reintegration2Digest := lateResurrection2Digest + "|rollback-fence-post-late-resurrection-quarantine-reintegration-epoch=6"
	reintegrationSealBoundaryDigest := reintegration2Digest + "|rollback-fence-post-late-resurrection-reintegration-seal-epoch=8"
	reintegrationSealDrift2Digest := reintegrationSealBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-epoch=10"
	reintegrationSealDriftReanchor2Digest := reintegrationSealDrift2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-epoch=12"
	reintegrationSealDriftReanchorCompaction2Digest := reintegrationSealDriftReanchor2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-epoch=14"
	reintegrationSealDriftReanchorCompactionExpiry2Digest := reintegrationSealDriftReanchorCompaction2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-epoch=16"
	reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest := reintegrationSealDriftReanchorCompactionExpiry2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=18"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=20"
	reintegrationSeal2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=22"

	reintegrationSeal2Cfg := segment2Cfg
	reintegrationSeal2Cfg.PolicyManifestDigest = reintegrationSeal2Digest
	reintegrationSealDrift1Cfg := segment2Cfg
	reintegrationSealDrift1Cfg.PolicyManifestDigest = reintegrationSeal2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=23"
	reintegrationSealDrift2Cfg := segment2Cfg
	reintegrationSealDrift2Cfg.PolicyManifestDigest = reintegrationSeal2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=24"
	staleReintegrationSealDriftCfg := segment2Cfg
	...
	conflictingReintegrationSealDriftAliasForwardCfg := segment2Cfg
	conflictingReintegrationSealDriftAliasForwardCfg.PolicyManifestDigest = reintegrationSeal2Digest +
		"|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=24|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=23"
	conflictingReintegrationSealDriftAliasReverseCfg := segment2Cfg
	conflictingReintegrationSealDriftAliasReverseCfg.PolicyManifestDigest = reintegrationSeal2Digest +
		"|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=23|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=24"
	...
	assert.Equal(
		...
		reintegrationSealDrift2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftDecision.PolicyManifestDigest,
		"lower reintegration-seal drift-reanchor lineage-compaction marker-expiry late-resurrection-quarantine-reintegration-seal-drift epochs must remain pinned behind latest verified drift ownership",
	)
	assert.Equal(t, reintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftDecision.PolicyActivationTicks)

	staleReintegrationSealEchoSeed := staleReintegrationSealDriftController.currentBatch
	staleReintegrationSealEchoController := newAutoTuneControllerWithSeed(100, staleReintegrationSealEchoCfg, &staleReintegrationSealEchoSeed)
	require.NotNil(t, staleReintegrationSealEchoController)
	staleReintegrationSealEchoController.reconcilePolicyTransition(staleReintegrationSealDriftController.exportPolicyTransition())
	batch, staleReintegrationSealEchoDecision := staleReintegrationSealEchoController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSealEchoSeed+20, batch)
	assert.Equal(t, "apply_increase", staleReintegrationSealEchoDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDrift2Cfg.PolicyManifestDigest,
		staleReintegrationSealEchoDecision.PolicyManifestDigest,
		"delayed reintegration-seal echoes must stay pinned behind verified reintegration-seal-drift ownership",
	)
	...
	assert.Equal(
		...
		reintegrationSealDrift2Cfg.PolicyManifestDigest,
		ambiguousReintegrationSealDriftDecision.PolicyManifestDigest,
		"reintegration-seal-drift candidates must remain quarantined until reintegration-seal ownership is explicit",
	)
	assert.Equal(t, reintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationSealDriftDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationSealDriftDecision.PolicyActivationTicks)

	conflictingForwardSeed := ambiguousReintegrationSealDriftController.currentBatch
	conflictingForwardController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealDriftAliasForwardCfg, &conflictingForwardSeed)
	require.NotNil(t, conflictingForwardController)
	conflictingForwardController.reconcilePolicyTransition(ambiguousReintegrationSealDriftController.exportPolicyTransition())
	batch, conflictingForwardDecision := conflictingForwardController.Resolve(highLag)
	assert.Equal(t, conflictingForwardSeed+20, batch)
	assert.Equal(t, "apply_increase", conflictingForwardDecision.Decision)
	assert.Equal(
	...
	assert.Equal(t, reintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, conflictingForwardDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingForwardDecision.PolicyActivationTicks)

	conflictingReverseSeed := conflictingForwardController.currentBatch
	conflictingReverseController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealDriftAliasReverseCfg, &conflictingReverseSeed)
	require.NotNil(t, conflictingReverseController)
	conflictingReverseController.reconcilePolicyTransition(conflictingForwardController.exportPolicyTransition())
	batch, conflictingReverseDecision := conflictingReverseController.Resolve(highLag)
	assert.Equal(t, conflictingReverseSeed+20, batch)
	...
	assert.Equal(t, reintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, conflictingReverseDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingReverseDecision.PolicyActivationTicks)

	crashRestartSeed := conflictingReverseController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealDrift2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
	...
	reForwardSeed := conflictingReverseController.currentBatch
	reForwardController := newAutoTuneControllerWithSeed(100, segment3Cfg, &reForwardSeed)
	require.NotNil(t, reForwardController)
	reForwardController.reconcilePolicyTransition(conflictingReverseController.exportPolicyTransition())
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal-drift must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
	assert.Equal(t, segment3Cfg.PolicyManifestRefreshEpoch, reForwardHold.PolicyEpoch)
	assert.Equal(t, 1, reForwardHold.PolicyActivationTicks)
}

func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftReanchorRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	reintegration2Digest := lateResurrection2Digest + "|rollback-fence-post-late-resurrection-quarantine-reintegration-epoch=6"
	reintegrationSealBoundaryDigest := reintegration2Digest + "|rollback-fence-post-late-resurrection-reintegration-seal-epoch=8"
	reintegrationSealDriftBoundaryDigest := reintegrationSealBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-epoch=10"
	reintegrationSealDriftReanchorBoundaryDigest := reintegrationSealDriftBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-epoch=12"
	reintegrationSealDriftReanchorCompactionBoundaryDigest := reintegrationSealDriftReanchorBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-epoch=14"
	reintegrationSealDriftReanchorCompactionExpiryBoundaryDigest := reintegrationSealDriftReanchorCompactionBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-epoch=16"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineBoundaryDigest := reintegrationSealDriftReanchorCompactionExpiryBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=18"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationBoundaryDigest := reintegrationSealDriftReanchorCompactionExpiryQuarantineBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=20"
	reintegrationSeal2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=22"
	reintegrationSealDrift2Digest := reintegrationSeal2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=24"

	reintegrationSealDrift2Cfg := segment2Cfg
	reintegrationSealDrift2Cfg.PolicyManifestDigest = reintegrationSealDrift2Digest
	reintegrationSealDriftReanchor1Cfg := segment2Cfg
	reintegrationSealDriftReanchor1Cfg.PolicyManifestDigest = reintegrationSealDrift2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=25"
	reintegrationSealDriftReanchor2Cfg := segment2Cfg
	reintegrationSealDriftReanchor2Cfg.PolicyManifestDigest = reintegrationSealDrift2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=26"
	staleReintegrationSealDriftReanchorCfg := segment2Cfg
	staleReintegrationSealDriftReanchorCfg.PolicyManifestDigest = reintegrationSealDrift2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=25"
	staleReintegrationSealDriftEchoCfg := segment2Cfg
	staleReintegrationSealDriftEchoCfg.PolicyManifestDigest = reintegrationSeal2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=27"
	ambiguousReintegrationSealDriftReanchorCfg := segment2Cfg
	ambiguousReintegrationSealDriftReanchorCfg.PolicyManifestDigest = reintegrationSeal2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=28"
	conflictingReintegrationSealDriftReanchorAliasForwardCfg := segment2Cfg
	conflictingReintegrationSealDriftReanchorAliasForwardCfg.PolicyManifestDigest = reintegrationSealDrift2Digest +
		"|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=26|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=25"
	conflictingReintegrationSealDriftReanchorAliasReverseCfg := segment2Cfg
	conflictingReintegrationSealDriftReanchorAliasReverseCfg.PolicyManifestDigest = reintegrationSealDrift2Digest +
		"|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=25|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=26"
	...
	staleReintegrationSealDriftEchoSeed := staleReintegrationSealDriftReanchorController.currentBatch
	staleReintegrationSealDriftEchoController := newAutoTuneControllerWithSeed(100, staleReintegrationSealDriftEchoCfg, &staleReintegrationSealDriftEchoSeed)
	require.NotNil(t, staleReintegrationSealDriftEchoController)
	staleReintegrationSealDriftEchoController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorController.exportPolicyTransition())
	batch, staleReintegrationSealDriftEchoDecision := staleReintegrationSealDriftEchoController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSealDriftEchoSeed+20, batch)
	assert.Equal(t, "apply_increase", staleReintegrationSealDriftEchoDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchor2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftEchoDecision.PolicyManifestDigest,
		"delayed reintegration-seal-drift echoes must stay pinned behind verified reintegration-seal-drift-reanchor ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchor2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftEchoDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftEchoDecision.PolicyActivationTicks)

	ambiguousReintegrationSealDriftReanchorSeed := staleReintegrationSealDriftEchoController.currentBatch
	ambiguousReintegrationSealDriftReanchorController := newAutoTuneControllerWithSeed(100, ambiguousReintegrationSealDriftReanchorCfg, &ambiguousReintegrationSealDriftReanchorSeed)
	require.NotNil(t, ambiguousReintegrationSealDriftReanchorController)
	ambiguousReintegrationSealDriftReanchorController.reconcilePolicyTransition(staleReintegrationSealDriftEchoController.exportPolicyTransition())
	batch, ambiguousReintegrationSealDriftReanchorDecision := ambiguousReintegrationSealDriftReanchorController.Resolve(highLag)
	assert.Equal(t, ambiguousReintegrationSealDriftReanchorSeed+20, batch)
	assert.Equal(t, "apply_increase", ambiguousReintegrationSealDriftReanchorDecision.Decision)
	assert.Equal(
		...
		reintegrationSealDriftReanchor2Cfg.PolicyManifestDigest,
		ambiguousReintegrationSealDriftReanchorDecision.PolicyManifestDigest,
		"reintegration-seal-drift-reanchor candidates must remain quarantined until reintegration-seal-drift ownership is explicit",
	)
	assert.Equal(t, reintegrationSealDriftReanchor2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationSealDriftReanchorDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationSealDriftReanchorDecision.PolicyActivationTicks)

	conflictingForwardSeed := ambiguousReintegrationSealDriftReanchorController.currentBatch
	conflictingForwardController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealDriftReanchorAliasForwardCfg, &conflictingForwardSeed)
	require.NotNil(t, conflictingForwardController)
	conflictingForwardController.reconcilePolicyTransition(ambiguousReintegrationSealDriftReanchorController.exportPolicyTransition())
	batch, conflictingForwardDecision := conflictingForwardController.Resolve(highLag)
	assert.Equal(t, conflictingForwardSeed+20, batch)
	assert.Equal(t, "apply_increase", conflictingForwardDecision.Decision)
	assert.Equal(
	...
	assert.Equal(t, reintegrationSealDriftReanchor2Cfg.PolicyManifestRefreshEpoch, conflictingForwardDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingForwardDecision.PolicyActivationTicks)

	conflictingReverseSeed := conflictingForwardController.currentBatch
	conflictingReverseController := newAutoTuneControllerWithSeed(100, conflictingReintegrationSealDriftReanchorAliasReverseCfg, &conflictingReverseSeed)
	require.NotNil(t, conflictingReverseController)
	conflictingReverseController.reconcilePolicyTransition(conflictingForwardController.exportPolicyTransition())
	batch, conflictingReverseDecision := conflictingReverseController.Resolve(highLag)
	assert.Equal(t, conflictingReverseSeed+20, batch)
	...
	assert.Equal(t, reintegrationSealDriftReanchor2Cfg.PolicyManifestRefreshEpoch, conflictingReverseDecision.PolicyEpoch)
	assert.Equal(t, 0, conflictingReverseDecision.PolicyActivationTicks)
...
func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftReanchorLineageCompactionRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	reintegrationSeal2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationBoundaryDigest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=22"
	reintegrationSealDrift2Digest := reintegrationSeal2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=24"
	reintegrationSealDriftReanchor2Digest := reintegrationSealDrift2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=26"

	reintegrationSealDriftReanchor2Cfg := segment2Cfg
	reintegrationSealDriftReanchor2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchor2Digest
	reintegrationSealDriftReanchorCompaction1Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompaction1Cfg.PolicyManifestDigest = reintegrationSealDriftReanchor2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-epoch=27"
	reintegrationSealDriftReanchorCompaction2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompaction2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchor2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-epoch=28"
	staleReintegrationSealDriftReanchorCompactionCfg := segment2Cfg
	staleReintegrationSealDriftReanchorCompactionCfg.PolicyManifestDigest = reintegrationSealDriftReanchor2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-epoch=27"
	staleReintegrationSealDriftReanchorEchoCfg := segment2Cfg
	staleReintegrationSealDriftReanchorEchoCfg.PolicyManifestDigest = reintegrationSealDrift2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=29"
	ambiguousReintegrationSealDriftReanchorCompactionCfg := segment2Cfg
	ambiguousReintegrationSealDriftReanchorCompactionCfg.PolicyManifestDigest = reintegrationSealDrift2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-epoch=30"
	...
	staleReintegrationSealDriftReanchorEchoSeed := staleReintegrationSealDriftReanchorCompactionController.currentBatch
	staleReintegrationSealDriftReanchorEchoController := newAutoTuneControllerWithSeed(100, staleReintegrationSealDriftReanchorEchoCfg, &staleReintegrationSealDriftReanchorEchoSeed)
	require.NotNil(t, staleReintegrationSealDriftReanchorEchoController)
	staleReintegrationSealDriftReanchorEchoController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorCompactionController.exportPolicyTransition())
	batch, staleReintegrationSealDriftReanchorEchoDecision := staleReintegrationSealDriftReanchorEchoController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSealDriftReanchorEchoSeed+20, batch)
	assert.Equal(t, "apply_increase", staleReintegrationSealDriftReanchorEchoDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompaction2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftReanchorEchoDecision.PolicyManifestDigest,
		"delayed reintegration-seal-drift-reanchor echoes must stay pinned behind verified reintegration-seal-drift-reanchor-lineage-compaction ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompaction2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftReanchorEchoDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftReanchorEchoDecision.PolicyActivationTicks)

	ambiguousReintegrationSealDriftReanchorCompactionSeed := staleReintegrationSealDriftReanchorEchoController.currentBatch
	ambiguousReintegrationSealDriftReanchorCompactionController := newAutoTuneControllerWithSeed(100, ambiguousReintegrationSealDriftReanchorCompactionCfg, &ambiguousReintegrationSealDriftReanchorCompactionSeed)
	require.NotNil(t, ambiguousReintegrationSealDriftReanchorCompactionController)
	ambiguousReintegrationSealDriftReanchorCompactionController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorEchoController.exportPolicyTransition())
	batch, ambiguousReintegrationSealDriftReanchorCompactionDecision := ambiguousReintegrationSealDriftReanchorCompactionController.Resolve(highLag)
	assert.Equal(t, ambiguousReintegrationSealDriftReanchorCompactionSeed+20, batch)
	assert.Equal(t, "apply_increase", ambiguousReintegrationSealDriftReanchorCompactionDecision.Decision)
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompaction2Cfg.PolicyManifestDigest,
		ambiguousReintegrationSealDriftReanchorCompactionDecision.PolicyManifestDigest,
		"reintegration-seal-drift-reanchor-lineage-compaction candidates must remain quarantined until reintegration-seal-drift-reanchor ownership is explicit",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompaction2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationSealDriftReanchorCompactionDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationSealDriftReanchorCompactionDecision.PolicyActivationTicks)

	crashRestartSeed := ambiguousReintegrationSealDriftReanchorCompactionController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompaction2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
	...
	reForwardSeed := ambiguousReintegrationSealDriftReanchorCompactionController.currentBatch
	reForwardController := newAutoTuneControllerWithSeed(100, segment3Cfg, &reForwardSeed)
	require.NotNil(t, reForwardController)
	reForwardController.reconcilePolicyTransition(ambiguousReintegrationSealDriftReanchorCompactionController.exportPolicyTransition())
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal-drift-reanchor-lineage-compaction must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
...
func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftReanchorLineageCompactionMarkerExpiryRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	staleReintegrationSealDriftReanchorCompactionEchoCfg := segment2Cfg
	...
	ambiguousReintegrationSealDriftReanchorCompactionExpiryCfg := segment2Cfg
	ambiguousReintegrationSealDriftReanchorCompactionExpiryCfg.PolicyManifestDigest = reintegrationSealDriftReanchor2Cfg.PolicyManifestDigest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-epoch=32"
	...
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompactionExpiry2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftReanchorCompactionExpiryDecision.PolicyManifestDigest,
		"lower reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry epochs must remain pinned behind latest verified ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiry2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftReanchorCompactionExpiryDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftReanchorCompactionExpiryDecision.PolicyActivationTicks)

	staleReintegrationSealDriftReanchorCompactionEchoSeed := staleReintegrationSealDriftReanchorCompactionExpiryController.currentBatch
	staleReintegrationSealDriftReanchorCompactionEchoController := newAutoTuneControllerWithSeed(100, staleReintegrationSealDriftReanchorCompactionEchoCfg, &staleReintegrationSealDriftReanchorCompactionEchoSeed)
	require.NotNil(t, staleReintegrationSealDriftReanchorCompactionEchoController)
	staleReintegrationSealDriftReanchorCompactionEchoController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorCompactionExpiryController.exportPolicyTransition())
	batch, staleReintegrationSealDriftReanchorCompactionEchoDecision := staleReintegrationSealDriftReanchorCompactionEchoController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSealDriftReanchorCompactionEchoSeed+20, batch)
	assert.Equal(t, "apply_increase", staleReintegrationSealDriftReanchorCompactionEchoDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiry2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftReanchorCompactionEchoDecision.PolicyManifestDigest,
		"delayed reintegration-seal-drift-reanchor-lineage-compaction echoes must stay pinned behind verified marker-expiry ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiry2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftReanchorCompactionEchoDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftReanchorCompactionEchoDecision.PolicyActivationTicks)

	ambiguousReintegrationSealDriftReanchorCompactionExpirySeed := staleReintegrationSealDriftReanchorCompactionEchoController.currentBatch
	ambiguousReintegrationSealDriftReanchorCompactionExpiryController := newAutoTuneControllerWithSeed(100, ambiguousReintegrationSealDriftReanchorCompactionExpiryCfg, &ambiguousReintegrationSealDriftReanchorCompactionExpirySeed)
	require.NotNil(t, ambiguousReintegrationSealDriftReanchorCompactionExpiryController)
	ambiguousReintegrationSealDriftReanchorCompactionExpiryController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorCompactionEchoController.exportPolicyTransition())
	batch, ambiguousReintegrationSealDriftReanchorCompactionExpiryDecision := ambiguousReintegrationSealDriftReanchorCompactionExpiryController.Resolve(highLag)
	assert.Equal(t, ambiguousReintegrationSealDriftReanchorCompactionExpirySeed+20, batch)
	assert.Equal(t, "apply_increase", ambiguousReintegrationSealDriftReanchorCompactionExpiryDecision.Decision)
	assert.Equal(
	...
	crashRestartSeed := ambiguousReintegrationSealDriftReanchorCompactionExpiryController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiry2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
	...
	reForwardSeed := ambiguousReintegrationSealDriftReanchorCompactionExpiryController.currentBatch
	reForwardController := newAutoTuneControllerWithSeed(100, segment3Cfg, &reForwardSeed)
	require.NotNil(t, reForwardController)
	reForwardController.reconcilePolicyTransition(ambiguousReintegrationSealDriftReanchorCompactionExpiryController.exportPolicyTransition())
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
...
func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	staleReintegrationSealDriftReanchorCompactionExpiryEchoCfg := segment2Cfg
	...
	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineCfg := segment2Cfg
	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineCfg.PolicyManifestDigest = reintegrationSealDriftReanchor2Cfg.PolicyManifestDigest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=34"
	...
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.PolicyManifestDigest,
		"lower reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry-late-resurrection-quarantine epochs must remain pinned behind latest verified ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.PolicyActivationTicks)

	staleReintegrationSealDriftReanchorCompactionExpiryEchoSeed := staleReintegrationSealDriftReanchorCompactionExpiryQuarantineController.currentBatch
	staleReintegrationSealDriftReanchorCompactionExpiryEchoController := newAutoTuneControllerWithSeed(100, staleReintegrationSealDriftReanchorCompactionExpiryEchoCfg, &staleReintegrationSealDriftReanchorCompactionExpiryEchoSeed)
	require.NotNil(t, staleReintegrationSealDriftReanchorCompactionExpiryEchoController)
	staleReintegrationSealDriftReanchorCompactionExpiryEchoController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorCompactionExpiryQuarantineController.exportPolicyTransition())
	batch, staleReintegrationSealDriftReanchorCompactionExpiryEchoDecision := staleReintegrationSealDriftReanchorCompactionExpiryEchoController.Resolve(highLag)
	assert.Equal(t, staleReintegrationSealDriftReanchorCompactionExpiryEchoSeed+20, batch)
	assert.Equal(t, "apply_increase", staleReintegrationSealDriftReanchorCompactionExpiryEchoDecision.Decision)
	assert.Equal(
		t,
		reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftReanchorCompactionExpiryEchoDecision.PolicyManifestDigest,
		"delayed reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry echoes must stay pinned behind verified late-resurrection-quarantine ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftReanchorCompactionExpiryEchoDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftReanchorCompactionExpiryEchoDecision.PolicyActivationTicks)

	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineSeed := staleReintegrationSealDriftReanchorCompactionExpiryEchoController.currentBatch
	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController := newAutoTuneControllerWithSeed(100, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineCfg, &ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineSeed)
	require.NotNil(t, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController)
	ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController.reconcilePolicyTransition(staleReintegrationSealDriftReanchorCompactionExpiryEchoController.exportPolicyTransition())
	batch, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision := ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController.Resolve(highLag)
	assert.Equal(t, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineSeed+20, batch)
	assert.Equal(t, "apply_increase", ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.Decision)
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestDigest,
		ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.PolicyManifestDigest,
		"reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry-late-resurrection-quarantine candidates must remain quarantined until marker-expiry ownership is explicit",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineDecision.PolicyActivationTicks)

	crashRestartSeed := ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantine2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
	...
	reForwardSeed := ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController.currentBatch
	reForwardController := newAutoTuneControllerWithSeed(100, segment3Cfg, &reForwardSeed)
	require.NotNil(t, reForwardController)
	reForwardController.reconcilePolicyTransition(ambiguousReintegrationSealDriftReanchorCompactionExpiryQuarantineController.exportPolicyTransition())
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry-late-resurrection-quarantine must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
...
func TestAutoTuneController_RollbackCheckpointFencePostReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftReanchorLineageCompactionMarkerExpiryLateResurrectionQuarantineReintegrationSealDriftRejectsStaleMarkers(t *testing.T) {
	highLag := autoTuneInputs{
		HasHeadSignal:      true,
		HeadSequence:       1_000,
		HasMinCursorSignal: true,
	...
	reintegrationSealDrift2Digest := reintegrationSeal2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=24"
	reintegrationSealDriftReanchor2Digest := reintegrationSealDrift2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-epoch=26"
	reintegrationSealDriftReanchorCompaction2Digest := reintegrationSealDriftReanchor2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-epoch=28"
	reintegrationSealDriftReanchorCompactionExpiry2Digest := reintegrationSealDriftReanchorCompaction2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-epoch=30"
	reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest := reintegrationSealDriftReanchorCompactionExpiry2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-epoch=32"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantine2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-epoch=34"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=35"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Digest := reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=36"

	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Digest
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Digest
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift1Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift1Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=37"
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg := segment2Cfg
	reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=38"
	staleReintegrationSealDriftCfg := segment2Cfg
	staleReintegrationSealDriftCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=37"
	staleReintegrationSealEchoCfg := segment2Cfg
	staleReintegrationSealEchoCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=39"
	ambiguousReintegrationSealDriftCfg := segment2Cfg
	ambiguousReintegrationSealDriftCfg.PolicyManifestDigest = reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegration2Digest + "|rollback-fence-resurrection-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=40"
	...
	reintegrationSeal1Seed := baseController.currentBatch
	reintegrationSeal1Controller := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Cfg, &reintegrationSeal1Seed)
	require.NotNil(t, reintegrationSeal1Controller)
	reintegrationSeal1Controller.reconcilePolicyTransition(baseController.exportPolicyTransition())
	batch, reintegrationSeal1Decision := reintegrationSeal1Controller.Resolve(highLag)
	assert.Equal(t, reintegrationSeal1Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegrationSeal1Decision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Cfg.PolicyManifestDigest, reintegrationSeal1Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal1Cfg.PolicyManifestRefreshEpoch, reintegrationSeal1Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegrationSeal1Decision.PolicyActivationTicks)

	reintegrationSeal2Seed := reintegrationSeal1Controller.currentBatch
	reintegrationSeal2Controller := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Cfg, &reintegrationSeal2Seed)
	require.NotNil(t, reintegrationSeal2Controller)
	reintegrationSeal2Controller.reconcilePolicyTransition(reintegrationSeal1Controller.exportPolicyTransition())
	batch, reintegrationSeal2Decision := reintegrationSeal2Controller.Resolve(highLag)
	assert.Equal(t, reintegrationSeal2Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegrationSeal2Decision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Cfg.PolicyManifestDigest, reintegrationSeal2Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSeal2Cfg.PolicyManifestRefreshEpoch, reintegrationSeal2Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegrationSeal2Decision.PolicyActivationTicks)

	reintegrationSealDrift1Seed := reintegrationSeal2Controller.currentBatch
	reintegrationSealDrift1Controller := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift1Cfg, &reintegrationSealDrift1Seed)
	require.NotNil(t, reintegrationSealDrift1Controller)
	reintegrationSealDrift1Controller.reconcilePolicyTransition(reintegrationSeal2Controller.exportPolicyTransition())
	batch, reintegrationSealDrift1Decision := reintegrationSealDrift1Controller.Resolve(highLag)
	assert.Equal(t, reintegrationSealDrift1Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegrationSealDrift1Decision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift1Cfg.PolicyManifestDigest, reintegrationSealDrift1Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift1Cfg.PolicyManifestRefreshEpoch, reintegrationSealDrift1Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegrationSealDrift1Decision.PolicyActivationTicks)

	reintegrationSealDrift2Seed := reintegrationSealDrift1Controller.currentBatch
	reintegrationSealDrift2Controller := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg, &reintegrationSealDrift2Seed)
	require.NotNil(t, reintegrationSealDrift2Controller)
	reintegrationSealDrift2Controller.reconcilePolicyTransition(reintegrationSealDrift1Controller.exportPolicyTransition())
	batch, reintegrationSealDrift2Decision := reintegrationSealDrift2Controller.Resolve(highLag)
	assert.Equal(t, reintegrationSealDrift2Seed+20, batch)
	assert.Equal(t, "apply_increase", reintegrationSealDrift2Decision.Decision)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestDigest, reintegrationSealDrift2Decision.PolicyManifestDigest)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, reintegrationSealDrift2Decision.PolicyEpoch)
	assert.Equal(t, 0, reintegrationSealDrift2Decision.PolicyActivationTicks)
	...
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestDigest,
		staleReintegrationSealDriftDecision.PolicyManifestDigest,
		"lower reintegration-seal-drift-reanchor-lineage-compaction-marker-expiry-late-resurrection-quarantine-reintegration-seal-drift epochs must remain pinned behind latest verified ownership",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealDriftDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealDriftDecision.PolicyActivationTicks)
	...
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, staleReintegrationSealEchoDecision.PolicyEpoch)
	assert.Equal(t, 0, staleReintegrationSealEchoDecision.PolicyActivationTicks)
	...
	assert.Equal(
		...
		reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestDigest,
		ambiguousReintegrationSealDriftDecision.PolicyManifestDigest,
		"reintegration-seal-drift candidates must remain quarantined until explicit reintegration-seal ownership is present",
	)
	assert.Equal(t, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch, ambiguousReintegrationSealDriftDecision.PolicyEpoch)
	assert.Equal(t, 0, ambiguousReintegrationSealDriftDecision.PolicyActivationTicks)

	crashRestartSeed := ambiguousReintegrationSealDriftController.currentBatch
	crashRestartController := newAutoTuneControllerWithSeed(100, reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg, &crashRestartSeed)
	require.NotNil(t, crashRestartController)
	crashRestartController.reconcilePolicyTransition(autoTunePolicyTransition{
		HasState:                true,
		Version:                 reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyVersion,
		ManifestDigest:          reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestDigest,
		Epoch:                   reintegrationSealDriftReanchorCompactionExpiryQuarantineReintegrationSealDrift2Cfg.PolicyManifestRefreshEpoch,
		ActivationHoldRemaining: 2,
		FromWarmCheckpoint:      true,
	})
	batch, crashRestartHold := crashRestartController.Resolve(highLag)
	...
	reForwardSeed := ambiguousReintegrationSealDriftController.currentBatch
	reForwardController := newAutoTuneControllerWithSeed(100, segment3Cfg, &reForwardSeed)
	require.NotNil(t, reForwardController)
	reForwardController.reconcilePolicyTransition(ambiguousReintegrationSealDriftController.exportPolicyTransition())
	batch, reForwardHold := reForwardController.Resolve(highLag)
	assert.Equal(t, reForwardSeed, batch)
	assert.Equal(t, "hold_policy_transition", reForwardHold.Decision, "rollback+re-forward after reintegration-seal-drift must deterministically apply one activation hold")
	assert.Equal(t, segment3Cfg.PolicyManifestDigest, reForwardHold.PolicyManifestDigest)
...
func BenchmarkParseRollbackFenceOwnershipOrdering(b *testing.B) {
	...
	digest := "manifest-tail-v2b" +
		...
		"|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-epoch=36" +
		"|rollback-fence-post-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-reanchor-compaction-expiry-quarantine-reintegration-seal-drift-epoch=38"
