...
func TestCallBatch_Success(t *testing.T) {
	client := newTestClient(func(r *http.Request) (*http.Response, error) {
		...
		resp := []Response{
			{JSONRPC: "2.0", ID: reqs[1].ID, Result: json.RawMessage(`2`)},
			{JSONRPC: "2.0", ID: reqs[0].ID, Result: json.RawMessage(`1`)},
		}
		rawResp, err := json.Marshal(resp)
		require.NoError(t, err)
		return jsonHTTPResponse(http.StatusOK, string(rawResp)), nil
	...
	var v1 int
	require.NoError(t, json.Unmarshal(results[0].Result, &v1))
	assert.Equal(t, 1, v1)
	var v2 int
	require.NoError(t, json.Unmarshal(results[1].Result, &v2))
	assert.Equal(t, 2, v2)
}
